Ext.define('Xmin.util.Messages', {
    ...
    serverError: function(response) {
        if (response.status == 200) {
            ...
            this.userMessage("WARNING", gettext('Внимание'), app_response.msg);
        }
        else if (response.status == 404) {
            this.userMessage("ERROR", gettext('Ошибка сервера') + ' (' + response.status + ')',
                gettext('Запрашиваемый ресурс не найден или более не существует.'));
        }
        else if (response.status != 401) {
            this.userMessage("ERROR", gettext('Ошибка сервера') + ' (' + response.status + ')',
                gettext('Ошибка на сервере. <br> Пожалуйста, повторите попытку позже.'));
        }
    },
