...
class WorkflowTaskForm(forms.ModelForm):
    ...
    def get_url_params(self, instance=None):
        if instance:
            return "admin:{0}_change".format(self._meta.model._meta.db_table), [instance.pk]
        else:
            return "admin:{0}_add".format(self._meta.model._meta.db_table), []
...
class WorkflowTaskInstanceForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if instance:
            self.fields['performer'].widget.attrs.update({
                'suggest_url': '{0}'.format(reverse('admin:{0}_change'.format(
                    self._meta.model._meta.db_table), args=[instance.pk]))
            })
    ...
    def save(self, commit=True):
        ...
        if instance.state == WorkflowTaskInstance.State.OPEN and instance.closed_at:
            ...
            try:
                templates = WorkflowTask.objects.filter(order__gt=instance.task.order)
