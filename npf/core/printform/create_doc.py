...
class CreateDoc():
    ...
    def _replace_tags_name(self, name, fields_data):
        ...
        # tmptags = []
        # for tag in tags:
        #     tmptags.append(cleanhtml(tag))
        # print(repr(fields_data))
        new_text = name
        for tag in tags:
            if cleanhtml(tag) in fields_data:
                new_text = new_text.replace('[[' + tag + ']]', str(fields_data[cleanhtml(tag)]))
    ...
    def run(self):
        ...
        for id in ids:
            ...
            # doc_name = self.out_path + '\\' + template_name + ' '
            # + self.model._meta.verbose_name + ' - ' + \
            #           str(self.model.objects.get(id=id)) + ' (' + id + ')' + '.docx'
        ...
        # tmptmp = self._replace_tags_name(file_name, fields_data)
        # print(tmptmp)
        # print(fields_data)
        return files
